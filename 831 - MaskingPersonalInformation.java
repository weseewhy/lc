...
Note that extraneous characters like "(", ")", " ", as well as extra dashes or plus signs
...
class Solution {
    public String maskPII(String s) {
        ...
        if (i > 0) {
            ...
            String localNumber = "***-***-" + numbers.substring(numbers.length() - 4);
            if (numbers.length() == 10) {
                return localNumber;
            }

            StringBuilder internationalNumber = new StringBuilder("+");
            for (int j = 0; j < numbers.length() - 10; j++) {
                internationalNumber.append("*");
            }
            internationalNumber.append("-").append(localNumber);

            return internationalNumber.toString();
        }
    }
}
