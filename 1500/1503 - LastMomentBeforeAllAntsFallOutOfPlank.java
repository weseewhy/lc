...
Note that the last moment when an ant was on the plank is t = 4 second, after that it falls immediately out of the plank. 
...
class Solution {
	public int getLastMoment(int n, int[] left, int[] right) {
		int maxDistanceForLeftMoving = 0;
		for (int i : left) {
			maxDistanceForLeftMoving = Math.max(maxDistanceForLeftMoving, i);
		}

		int maxDistanceForRightMoving = 0;
		for (int i : right) {
			maxDistanceForRightMoving = Math.max(maxDistanceForRightMoving, n - i);
		}

		return Math.max(maxDistanceForLeftMoving, maxDistanceForRightMoving);
	}
}
