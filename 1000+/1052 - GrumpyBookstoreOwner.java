    ...
    public int maxSatisfied(int[] customers, int[] grumpy, int X) {
        ...
        for (int i = 0; i < X; i++) {
            if (grumpy[i] == 0) {
                ...
                currentWindowGrumpy += customers[i];
            }
        }

        int maxWindowGrumpy = currentWindowGrumpy;
        ...
        for (int i = X; i < customers.length; i++) {
            ...
            if (grumpy[i - X] == 1) {
                currentWindowGrumpy -= customers[i - X];
            }

            maxWindowGrumpy = Math.max(currentWindowGrumpy, maxWindowGrumpy);
        }

        return totalHappy + maxWindowGrumpy;
    }
}
